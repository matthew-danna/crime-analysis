---
title: "JLC 445/696 Project 2 Template v2"
author: "My Name"
...
For Forecast 1, my analysis is based on robbery during day shift.
```{r}
dc.day <- subset(dc.data, dc.data$SHIFT == 'DAY' & dc.data$OFFENSE == 'ROBBERY')
...
#### Temporal topology for all prior November's, by day and hour
...
#### Hotspot map for all prior Novembers
```{r, echo=FALSE}
ggplot() + 
  geom_sf(data = dc.outline) +
  geom_sf(data = dc.roads.major) +
  geom_point(aes(x = LONGITUDE, y = LATITUDE), data = dc.november.day, alpha = 0.4, size = 0.5) + 
  theme_void() +
  facet_wrap(~ YEAR, nrow = 4)
```
...
For Forecast 2, my analysis is based on robbery during day shift.
```{r}
dc.evening <- subset(dc.data, dc.data$SHIFT == 'EVENING' & dc.data$OFFENSE == 'ROBBERY')
dc.november.evening <- subset(dc.evening, dc.evening$MONTH == '11')
```

```{r, echo=FALSE, warning=FALSE, message=FALSE}
crime.evening <- dc.evening %>%
  group_by(DATE) %>%
  summarise(COUNT = n())
...
nov1 <- as.Date("2023-11-01")
nov30 <- as.Date("2023-11-30")
start.evening <- as.numeric(nov1 - max(crime.evening$DATE))
end.evening <- as.numeric(nov30 - max(crime.evening$DATE))
...
forecast.evening <- c(forecast00, forecast01, forecast02, forecast03, forecast04, 
                  forecast05, forecast06, forecast07, forecast08, forecast09, 
                  forecast10, forecast11, forecast12, forecast13, forecast14, 
                  forecast15, forecast16, forecast17, forecast18, forecast19, 
                  forecast20, forecast21, forecast22, forecast23, forecast24, 
...
colnames(forecasts.tmp) <- "DAY"
forecasts.tmp$ID <- seq.int(nrow(forecasts.tmp))
forecasts.tmp$DATE <- forecasts.tmp$DAY + max(crime.evening$DATE)
...
forecasts.tmp <- forecasts.tmp[c(2,4)]
names(forecasts.tmp) <- c("ID", "YESTERDAY")
forecasts.evening <- forecasts.evening %>% 
  left_join(forecasts.tmp, by = "ID")
...
#### Temporal topology for all prior November's, by day and hour
```{r, echo=FALSE, message=FALSE, warning=FALSE}
crime.year.evening <- dc.november.evening %>%
  group_by(YEAR, DOW, HOUR) %>%
  summarise(COUNT = n())
crime.year.evening <- subset(crime.year.evening, !is.na(crime.year.evening$HOUR))

ggplot(crime.year.evening, aes(HOUR, DOW, fill = COUNT)) +
  geom_tile() + 
  scale_fill_gradient(low = "white", high = "red") +
  theme(legend.position = "none") +
  coord_fixed() +
...
ggplot() + 
  geom_sf(data = dc.outline) +
  geom_sf(data = dc.roads.major) +
  geom_point(aes(x = LONGITUDE, y = LATITUDE), data = dc.november.evening, alpha = 0.4, size = 0.5) + 
  theme_void() +
  facet_wrap(~ YEAR, nrow = 4)
```
