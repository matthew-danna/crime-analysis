...
The Final Project is an extension of [Project 3](https://docs.google.com/document/d/1YieHxCkKotVrPLA5_2ngvtTa10tPLpukANshYsnXurA/edit?usp=sharing), that also involves the integration of work you've done throughout this semester and beyond. Use the format provided in this document as much or as little as you'd like, as long as the required components are covered. 

If you want to get crazy with Markdown themes, [here's a great link](https://rpubs.com/ranydc/rmarkdown_themes).
...
2. Get the latest and greatest D.C. crime data, like we did in Projects 2 and 3:
```{r}
...
3. Separate the date, format it, and enrich the dataset with more temporal measures:
```{r}
...
4. Determine the total count and percentage of each crime type:
```{r}
dc.sum <- dc.data %>%
  group_by(OFFENSE) %>%
...
5. Use the table from #4 to find and filter the crime type(s) to forecast (choose your own!):
```{r}
group <- c("ROBBERY")
...
2. Graph actual crime with predicted crime:
```{r}
...
ONE LAST THING... the source code for creating this file can be [found here](https://github.com/matthew-danna/crime-analysis/blob/main/JLC%20445%20696/Final%20Project/JLC%20445%20696%20Final%20Project%20Template.Rmd).

Please [email me](mailto:danna@american.edu) with any questions.
