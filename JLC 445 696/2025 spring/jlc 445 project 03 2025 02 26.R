...
### at this point you determine which dataset is the best fit for modeling, 
### and then proceed
...
arima.data <- auto.arima(cleaned.data, 
                         d = 1, max.p = 10, max.q = 1, # manual inputs based on above calculations
                         seasonal = T)
summary(arima.data)
...
###: Graph option 2, Graph monthly forecasts, by upper bounds (95% CI):
...
ggplot(crime.day.time, aes(HOUR, DOW, fill = COUNT)) +
  ...
  theme(legend.position = "none") +
  coord_fixed() +
  scale_y_discrete(limits = c("Saturday","Friday","Thursday","Wednesday",
