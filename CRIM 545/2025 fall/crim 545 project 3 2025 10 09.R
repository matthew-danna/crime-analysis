...
ggplot() +
  ...
  geom_hex(aes(x = LONGITUDE, y = LATITUDE), data = dc1, bins = 12,
           alpha = 0.5) +
...
# Map, option 2
dc1.subset <- subset(dc1, dc1$MONTH == '1')
leaflet(dc1.subset) %>%
  addTiles() %>%
  addMarkers(lng = ~LONGITUDE, lat = ~LATITUDE, 
             popup = paste(
               "Crime Type: ", dc1.subset$OFFENSE, "<br>",
               "Date:", dc1.subset$DATE, "<br>",
               "Shift: ", dc1.subset$SHIFT), 
             clusterOptions = markerClusterOptions()) %>%
  addPolygons(data = dc.outline)

# Map, option 3
leaflet(subset(dc1, dc1$MONTH == '1')) %>%
  addTiles() %>%
  addMarkers(lng = ~LONGITUDE, lat = ~LATITUDE, 
             clusterOptions = markerClusterOptions()) %>%
  addPolygons(data = dc.outline)
